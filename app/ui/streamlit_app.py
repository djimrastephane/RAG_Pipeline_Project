...
API_KEY_INPUT = st.sidebar.text_input("API Key (optional)", value="", type="password")
DEMO_MODE = os.getenv("DEMO_MODE", "0") == "1"

# ── Sidebar: API + Ollama health ─────────────────────────────────────────────
try:
    _health = requests.get(f"{API_BASE}/api/v1/health", timeout=4).json()
    _ollama_ok = bool(_health.get("ollama_reachable", False))
    _gen_enabled = bool(_health.get("generation_enabled", False))
    _detail = str(_health.get("ollama_detail") or "")
    if not _gen_enabled:
        st.sidebar.caption("Generation: disabled (LOCAL_LLM_ENABLED=0)")
    elif _ollama_ok:
        st.sidebar.success(f"Ollama reachable ({_detail})")
    else:
        st.sidebar.warning(f"Ollama unreachable — {_detail}")
except Exception as _e:
    st.sidebar.error(f"API unreachable: {_e}")
...
def _auth_headers() -> dict:
    """Return X-API-Key header dict when a key is configured."""
    key = str(API_KEY_INPUT or "").strip()
    return {"X-API-Key": key} if key else {}


def api_get(path: str) -> dict:
    """Execute GET request against API and return JSON body."""
    r = requests.get(f"{API_BASE}{path}", headers=_auth_headers(), timeout=120)
    r.raise_for_status()
    return r.json()
...
def api_post_json(path: str, payload: dict) -> dict:
    ...
    r = requests.post(f"{API_BASE}{path}", json=payload, headers=_auth_headers(), timeout=120)
    r.raise_for_status()
    return r.json()
...
def api_post_upload(path: str, pdf_file, eval_file=None) -> dict:
    ...
    r = requests.post(f"{API_BASE}{path}", json=payload, headers=_auth_headers(), timeout=1800)
    r.raise_for_status()
    return r.json()
...
stats = None
eval_items = []
eval_label_type = "gold"
if doc_id:
    try:
        stats = api_get(f"/api/v1/docs/{doc_id}/stats")
        _eval_payload = api_get(f"/api/v1/docs/{doc_id}/eval-items")
        eval_items = _eval_payload.get("items", [])
        eval_label_type = str(_eval_payload.get("label_type") or "gold")
    except Exception as e:
...
if stats:
    ...
    if eval_label_type == "silver":
        st.info(
            "**Silver eval set** — queries were auto-generated by the pipeline and filtered to "
            "generation_confidence=1.0. Expected answers are pipeline predictions, not human-verified. "
            "Use for retrieval diagnostics only; do not treat as gold-standard accuracy scores.",
            icon="ℹ️",
        )
