...
for src_path in (project_root / "src", repo_root / "src"):
    if src_path.exists() and str(src_path) not in sys.path:
        sys.path.insert(0, str(src_path))

try:
    from scripts.retrieval_eval import get_retrieved_pages
except ModuleNotFoundError:
    from retrieval_eval import get_retrieved_pages
try:
    from rag_pdf.retrieval.hybrid_utils import BM25Index, tokenize
except ModuleNotFoundError:
    from src.rag_pdf.retrieval.hybrid_utils import BM25Index, tokenize
