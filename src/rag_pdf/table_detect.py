...
def count_numeric_tokens(line: str) -> int:
    """Count numeric tokens (integers, decimals, percentages, currency amounts) in a single text line."""
...
def is_small_financial_table(text: str) -> bool:
    """Return True if the text block looks like a small financial table (few rows, currency/percentage tokens, aligned columns)."""
    cfg = TABLE_DETECT_CFG
    lines = [l for l in text.splitlines() if l.strip()]
